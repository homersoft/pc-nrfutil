...
class BleDevice(object):
    ...
    def connect(self):
        """ Connect to device. """
        logger.debug("Connectiong to dbus bluez")
        try:
...
class BluezBleAdapter(object):
    ...
    def att_mtu_exchange(self, conn_handle, mtu=None):
        """ Set ATT MTU.
        Current implementation does not support change of ATT MTU.

        :param conn_handle: int, connection handle
        "param mtu: int, mtu to be set
        :return: int, ATT MTU
        """
        # Currently changing ATT MTU is not possible.
        self.db_conns[conn_handle].att_mtu = self.DEFAULT_ATT_MTU if not mtu else mtu
        return self.db_conns[conn_handle].att_mtu


    def data_length_update(self, conn_handle, mtu=None):
        """ Set ATT MTU.
        Current implementation does not support change of ATT MTU.
