...
def find_interfaces_with_name(interfaces, name):
  return [interface_name for interface_name in interfaces.keys() if interface_name == name]


def find_ble_adapters(objects):
  return [path for path, interfaces in objects.items() if any(find_interfaces_with_name(interfaces, BLUEZ_ADAPTER_IF))]


def find_ble_devices(objects):
  return [path for path, interfaces in objects.items() if any(find_interfaces_with_name(interfaces, BLUEZ_DEV_IF))]
...
class BleGattCharacteristic(object):
  ...
  def notification_handler(self, interface_name, changed_properties, invalidated_properties):
...
class BleDevice(object):
  ...
  def rssi(self):
    try:
      ...
      logging.warning("%s: %s", err.get_dbus_name(), err.get_dbus_message())
...
class BleAdapter(object):
  ...
  def first_adapter(cls):
    ...
    if not len(available_adapters):
      raise Value("Could not find any Bluetooth adapter.")
...
class BluezBleAdapter(object):
  ...
  def service_discovery(self, conn_handle, uuid=None):
    self.db_conns[conn_handle] = self._get_connection_by_conn_handle(conn_handle)
    self.db_conns[conn_handle].discover_services()

    return BLEGattStatusCode.success

  def _get_connection_by_conn_handle(self, conn_handle):
  ...
  def enable_notification(self, conn_handle, uuid):
    char = self._get_characteristic_with_uuid(self._get_connection_by_conn_handle(conn_handle), uuid)
